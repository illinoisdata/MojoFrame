...
fn read_small(data: UnsafePointer[UInt8], length: Int) -> U128:
    if length >= 2:
        if length >= 4:
            ...
            var b = data.offset(length - 4).bitcast[UInt32]().load().cast[DType.uint64]()
            return U128(a, b)
        else:
...
struct AHasher:
    ...
    fn write(inout self, data: UnsafePointer[UInt8], length: Int):
        ...
        if length > 8:
            if length > 16:
                var tail = data.offset(length - 16).bitcast[UInt64]().load[width=2]()
                self.large_update(tail)
                var offset = 0
                while length - offset > 16:
                    var block = data.offset(offset).bitcast[UInt64]().load[width=2]()
