...
alias FLOAT_VEC_WIDTH = simd_width_of[Float64]()
...
struct DataFrameF64:
    ...
    fn __init__(out self, owned columns: List[Float64Array], owned column_names: List[String]) raises:
        # var mem_before = self._get_process_memory()
        ...
        for i in range(len(self.column_names)):
            self.col_name_to_idx[self.column_names[i]] = i

        # var mem_after = self._get_process_memory()
        # self.creation_memory_mb = mem_after - mem_before
    ...
    fn __getitem__(self, column_name: String) raises -> Float64Array:
        return self.columns[self.col_name_to_idx[column_name]]

    fn __setitem__(mut self, i: Int, owned value: Float64Array) raises:
        self.columns[i] = value^

    # Column management
    fn append_column(mut self, owned new_column: Float64Array, new_column_name: String) raises:
        self.columns.append(new_column^)
        self.column_names.append(new_column_name)
        self.col_name_to_idx[new_column_name] = len(self.columns) - 1
    ...
    # GroupBy operations
    fn groupby(mut self, column: String, aggregation: String, aggregated_col_names: List[String]) raises:
        var groupby_col_idx = self.col_name_to_idx[column]
        
        if aggregation == "sum":
            self.columns = aggregation_sum_f64(self.columns, self.column_names, groupby_col_idx)
        elif aggregation == "mean":
            self.columns = aggregation_mean_f64(self.columns, self.column_names, groupby_col_idx)
        elif aggregation == "count":
            self.columns = aggregation_count_f64(self.columns, self.column_names, groupby_col_idx)
        elif aggregation == "min":
            self.columns = aggregation_min_f64(self.columns, self.column_names, groupby_col_idx)
        elif aggregation == "count_distinct":
            # the first column is the groups, the second column is the aggregation results
            self.columns = aggregation_count_distinct_f64_fast(self.columns, self.column_names, self.col_name_to_idx[column], self.col_name_to_idx[aggregated_col_names[1]])
        else:
            self.columns = aggregation_all_f64(self.columns, self.column_names, groupby_col_idx)

        self._update_column_mapping(aggregated_col_names)
    ...
    fn groupby_multicol_selective(mut self, group_cols: List[String], agg_cols: List[String], aggregation: String, result_col_names: List[String]) raises:
        if aggregation == "sum":
            self.columns = aggregation_sum_f64_multicol_selective(self, group_cols, agg_cols)
        
        self._update_column_mapping(result_col_names)
    ...
    fn select_complex(mut self, read masks: List[List[Bool]], logical_operator: String) raises:
        ...
        if logical_operator == "AND":
            for i in range(1, len(masks)):
                for j in range(mask_len):
                    filtered_mask[j] = filtered_mask[j] and masks[i][j]
        elif logical_operator == "OR":
            for i in range(1, len(masks)):
                for j in range(mask_len):
                    filtered_mask[j] = filtered_mask[j] or masks[i][j]
        else:
            raise Error("Unsupported logical operator: " + logical_operator)

        # Apply mask
        self._apply_boolean_mask(filtered_mask)

    fn select_mask[T: PredicateF64, T2: PredicateF64](
        self, column_1: String, column_2: String,
        predicate_1: T, predicate_2: T2,
        value_cmp_1: SIMD[DType.float64, 1], value_cmp_2: SIMD[DType.float64, 1],
        logical_operator: String
    ) raises -> List[Bool]:
            
        return evaluate_f64_mask(
    ...
    fn select[T: PredicateF64, T2: PredicateF64](
        mut self, column_1: String, column_2: String,
        predicate_1: T, predicate_2: T2,
        value_cmp_1: SIMD[DType.float64, 1], value_cmp_2: SIMD[DType.float64, 1],
        logical_operator: String
    ) raises:
        ...
        self._apply_index_selection(selected_indices)
    ...
    fn sort_by(mut self, by: List[String]) raises:
        ...
        if len(by) == 1:
            ...
            # Lexicographic sort: sort by rightmost key first, then leftmost
            for i in range(len(by) - 1, -1, -1):
                var key = self[by[i]]
                _ = mergesort(key, sorted_indexer)
        
        self._reorder_by_indices(sorted_indexer)

    # Private helper methods
    fn _update_column_mapping(mut self, new_column_names: List[String]):
        """Update column names and rebuild the name-to-index mapping."""
        self.column_names = new_column_names
        self.col_name_to_idx.clear()
        for i in range(len(self.column_names)):
            self.col_name_to_idx[self.column_names[i]] = i

    fn _apply_boolean_mask(mut self, mask: List[Bool]) raises:
        """Apply a boolean mask to filter rows."""
        var selected_indices = List[Int]()
        for i in range(len(mask)):
            if mask[i]:
                selected_indices.append(i)
        self._reorder_by_indices(selected_indices)

    fn _apply_index_selection(mut self, selected_indices: Int32Array) raises:
        """Apply index-based selection with parallel processing."""
        var filtered_rows_N = selected_indices.size
        var filtered_data = List[Float64Array]()
        ...
        for col_i in range(len(self.columns)):
            ...
            fn fill_data_worker(chunk_id: Int):
                ...
                # Vectorized copy
                for row_i in range(start_i, limit, FLOAT_VEC_WIDTH):
                    var filtered_idxs = selected_indices.data.load[width=FLOAT_VEC_WIDTH](row_i, 0)
                    var gathered_values = SIMD[DType.float64, FLOAT_VEC_WIDTH]()
                    ...
                    col_to_fill.data.store[FLOAT_VEC_WIDTH](row_i, 0, gathered_values)
    ...
    fn _reorder_by_indices(mut self, indices: List[Int]) raises:
        """Reorder all columns based on provided indices."""

        var reordered_data = List[Float64Array]()
        var new_size = len(indices)
        
        for col_i in range(len(self.columns)):
            var col_to_fill = Float64Array(new_size)
            for row_i in range(new_size):
                col_to_fill[row_i] = self.columns[col_i][indices[row_i]]
            reordered_data.append(col_to_fill^)
        
        self.columns = reordered_data^
    ...
    fn shape(self) -> Tuple[Int, Int]:
        """Return the shape (rows, cols) of the DataFrame."""
...
struct SetElement(Copyable, Movable):
