...
def fetch_dataset(path: str) -> pd.DataFrame:
    ...
    match FILE_TYPE:
        ...
        case "parquet":
            scan = pd.read_parquet(path)
        case "feather":
            scan = pd.read_feather(path)
        case _:
...
def get_query_answer(query_num: int, base_dir: str = ANSWERS_BASE_DIR) -> pd.DataFrame:
    ...
    answer_df = pd.read_csv(
        os.path.join(base_dir, f"{query_num}.csv"),
        sep=",",
        header=True,
    )
    columns: list[str] = list(answer_df.columns)
    answer_df = answer_df[[c.strip() for c in columns]]

    return answer_df


def test_results(query_num: int, result_df: pd.DataFrame) -> bool:
    """Test the results of TPC-H query number query_num

    Args:
        query_num (int): the TPC-H query number to test
        result_df (pd.DataFrame): the results from running
        the query
    ...
    with TPCHTimer(f"Query {query_num} result testing"):
        answer: pd.DataFrame = get_query_answer(query_num)
...
def get_line_item_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_orders_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_customer_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_region_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_nation_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_supplier_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_part_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def get_part_supp_ds(base_dir: str = DATASET_BASE_DIR) -> pd.DataFrame:
...
def run_query(query_num: int, result: pd.DataFrame):
    ...
    if SAVE_RESULTS:
        result.to_csv(f"{OUTPUT_BASE_DIR}/query{query_num}.csv")
