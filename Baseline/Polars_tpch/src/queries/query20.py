...
def q():
    ...
    part_subquery = part_ds.filter(pl.col("p_name").str.starts_with("forest")).select(
        "p_partkey"
    )
    ...
    ps_suppkey_subquery = (
        part_supp_ds.join(part_subquery, left_on="ps_partkey", right_on="p_partkey")
        .join(
            lineitem_group_ds,
    ...
    q_final = (
        ...
        .filter(
            pl.col("s_suppkey").is_in(
                pl.lit(ps_suppkey_subquery.get_column("ps_suppkey"))
            )
        )
